...
const BP = "/threat-console"; // T-Pot 리버스프록시 basePath (raw fetch/iframe는 Next가 자동 prefix 안 함)
...
export default function DashboardPage() {
  ...
  const 공격목록새로고침 = useCallback(() => {
    ...
    const f = 필터 === "attacks" || 필터 === "highrisk" ? 필터 : "all";
    fetch(`${BP}/api/attacks?view=${view}&filter=${f}`)
      .then((r) => r.json())
  ...
  const 게이지그리기 = useCallback((위험점수: number) => {
    ...
    ([{ s: 0, e: 34, c: "#22c55e" }, { s: 34, e: 67, c: "#f59e0b" }, { s: 67, e: 100, c: "#ef4444" }] as const).forEach(({ s, e, c }) => {
      g.append("path").attr("d", arc({ startAngle: 각도(s), endAngle: 각도(e) }) ?? "").attr("fill", c).attr("opacity", 0.18);
    });

    const 활성색 = 위험도 < 34 ? "#22c55e" : 위험도 < 67 ? "#f59e0b" : "#ef4444";
    ...
    활성arc.datum(d).attr("d", arc(d) ?? "")
      ...
      .attrTween("d", () => {
        const i = d3.interpolate(d.endAngle, 각도(위험도));
    ...
    바늘.transition().duration(900).ease(d3.easeCubicOut)
      .attrTween("x2", () => { const i = d3.interpolate(0, L * Math.sin(각도(위험도))); return (t) => String(i(t)); })
      .attrTween("y2", () => { const i = d3.interpolate(0, -L * Math.cos(각도(위험도))); return (t) => String(i(t)); });

    g.append("circle").attr("r", 6).attr("fill", "#1a1a2e");
    g.append("text").attr("text-anchor", "middle").attr("y", -16)
      .attr("font-size", "28px").attr("font-weight", "800").attr("fill", 활성색).text(위험도);
  ...
  async function 분석시작() {
    ...
    try {
      const res = await fetch(`${BP}/api/analyze/stream`, {
        method: "POST",
  ...
  const 위험도 = 선택공격 ? Math.round(선택공격.위험점수) : null;
  const 활성색 = 위험도 !== null ? (위험도 < 34 ? "#22c55e" : 위험도 < 67 ? "#f59e0b" : "#ef4444") : "#22c55e";
  const 위험라벨 = 위험도 !== null ? (위험도 < 34 ? "양호" : 위험도 < 67 ? "주의" : "위험") : "";
    ...
    <>
      ...
      <section className="맵섹션">
        <div className="카드" style={{ padding: "16px 20px 14px" }}>
          ...
          {맵펼침 && (
            <>
              <iframe src={`${BP}/tpot-map/`} title="T-Pot Attack Map" className="맵아이프레임" />
        ...
        <div className="왼쪽패널">
          ...
          {showGauge && 위험도 !== null && (
            <div className="카드" style={{ padding: "16px 20px 10px" }}>
              <div className="게이지카드제목"><ShieldAlert className="icon" /> 위험도</div>
              <svg ref={게이지SVGRef} id="게이지SVG" viewBox="0 0 220 115" style={{ display: "block", width: "100%", overflow: "visible" }} />
              <div className="게이지레이블">
                위험도: <strong style={{ color: 활성색 }}>{위험라벨}</strong> &nbsp;·&nbsp; 위험 점수 {선택공격?.위험점수.toFixed(1)}/100
              </div>
            </div>
          )}
